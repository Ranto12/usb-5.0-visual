...
export default function Portfolio({ portfolio: cmsPortfolio }: { portfolio: PortfolioItem[] }) {
  ...
  return (
    <section id="portfolio" className="section-padding bg-brand-dark/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="columns-2 md:columns-3 lg:columns-4 gap-3 space-y-3">
          {filtered.map((item, index) => (
            <div
              key={item.slug}
              className="break-inside-avoid group relative overflow-hidden rounded-xl cursor-pointer bg-brand-dark border border-white/5 transition-all duration-500"
            ...
            >
              ...
              >
                ...
                <div className="absolute top-2 right-2 flex flex-col items-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  <span className="bg-brand-red text-white text-xs font-semibold px-2 py-0.5 rounded-full capitalize">
                    {item.category}
                  </span>
                  {item.link_url && (
                    <a
                      ...
                      className="w-7 h-7 rounded-full bg-white/10 backdrop-blur border border-white/20 flex items-center justify-center text-white hover:bg-brand-red hover:border-brand-red transition-all duration-200"
                    >
      ...
      {lightbox && (
        ...
        >
          <button
            className="absolute top-4 right-4 text-white hover:text-brand-red transition-colors p-2"
            onClick={() => setLightbox(null)}
            aria-label="Close"
          >
            ...
            <div className="mt-4 text-center">
              ...
              {lightbox.link_url && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 mt-4 bg-brand-red hover:bg-red-700 text-white font-semibold px-5 py-2.5 rounded-full text-sm transition-colors"
                >
