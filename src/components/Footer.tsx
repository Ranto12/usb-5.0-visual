...
export default function Footer({ contactData }: { contactData: ContactData | null }) {
  ...
  return (
    <footer className="bg-brand-bone-dark border-t border-brand-bone-border">
      {/* Main footer */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {/* Brand */}
          <div className="lg:col-span-2">
            <button onClick={() => scrollTo('#home')} className="flex items-center justify-center group w-14 h-14 relative mb-4">
              <Image 
                src="/logo.png" 
                alt="USB-5.0 VISUALS Logo" 
                fill
                className="object-contain group-hover:scale-105 transition-transform duration-300"
              />
            </button>
            <p className="text-slate-600 text-caption mb-2 font-medium">{t.footer.tagline}</p>
            <p className="text-slate-500 text-caption leading-relaxed max-w-sm mb-6">{t.footer.description}</p>

            {/* Social Links */}
            <div>
              <p className="text-slate-500 text-overline font-semibold uppercase tracking-wider mb-3">{t.footer.followUs}</p>
              <div className="flex items-center gap-3">
                {socialLinks.map((social) => (
                  <a
                    ...
                    className="w-9 h-9 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-600 hover:text-white hover:border-brand-accent hover:bg-brand-accent transition-all duration-200 shadow-sm"
                  >
                    {social.icon}
                  </a>
                ))}
          ...
          {navSections.map((section) => (
            <div key={section.title}>
              <h4 className="text-slate-900 font-semibold text-sm mb-4 tracking-wide">{section.title}</h4>
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <button
                      onClick={() => scrollTo(link.href)}
                      className="text-slate-600 hover:text-slate-900 text-caption transition-colors text-left animated-underline"
                    >
                      {link.label}
                    </button>
                  </li>
      ...
      {/* CTA Banner */}
      <div className="border-t border-brand-bone-border bg-brand-accent/5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <p className="text-slate-900 font-semibold">Siap bekerja sama dengan kami?</p>
              <p className="text-slate-600 text-caption">Konsultasi gratis via WhatsApp sekarang</p>
            </div>
            <a
              href={`https://wa.me/${waNumber}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 bg-brand-accent hover:bg-brand-accent-hover text-white font-bold px-6 py-3 rounded-full text-sm transition-colors whitespace-nowrap shadow-sm"
            >
      ...
      {/* Bottom */}
      <div className="border-t border-brand-bone-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 text-slate-500 text-[0.6875rem]">
            <p>
